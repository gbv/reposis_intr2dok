...
<xsl:stylesheet version="1.0"
  ...
  <xsl:template match="/">
    ...
    <xsl:choose>
      <xsl:when
              test="/mycoreobject/metadata/def.modsContainer/modsContainer/mods:mods/mods:genre[@type='intern' and 'mir_genres#blog' = substring(@valueURI, string-length(@valueURI) - string-length('mir_genres#blog') +1)]">
        <div id="toc">
          <xsl:copy-of select="document(concat('staticcontent:mir-toc-blog:', /mycoreobject/@ID))/div/*"/>
        </div>
      </xsl:when>
      <xsl:otherwise>
     <!-- end blog toc hack -->
